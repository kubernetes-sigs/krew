...
func TestValidateReceipt(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:        "success",
			receiptName: "foo",
			receipt:     testutil.NewReceipt().WithName("foo").V(),
			wantErr:     false,
		},
		{
			name:        "file name mismatch",
			receiptName: "orange",
			receipt:     testutil.NewReceipt().WithName("apple").V(),
			wantErr:     true,
		},
		{
			name:        "bad api version",
			receiptName: "foo",
			receipt: testutil.NewReceipt().WithName("foo").WithTypeMeta(metav1.TypeMeta{
				APIVersion: "core/v1",
				Kind:       constants.PluginKind,
			}).V(),
			wantErr: true,
		},
		{
			name:        "wrong kind",
			receiptName: "foo",
			receipt: testutil.NewReceipt().WithName("foo").WithTypeMeta(
				metav1.TypeMeta{
					APIVersion: constants.CurrentAPIVersion,
					Kind:       "Not" + constants.PluginKind,
				}).V(),
		...
		},
		{
			name:        "shortDescription unspecified",
			receiptName: "foo",
			receipt:     testutil.NewReceipt().WithName("foo").WithShortDescription("").V(),
			wantErr:     true,
		},
		{
			name:        "version unspecified",
			receiptName: "foo",
			receipt:     testutil.NewReceipt().WithName("foo").WithVersion("").V(),
			wantErr:     true,
		},
		{
			name:        "version malformed",
			receiptName: "foo",
			receipt:     testutil.NewReceipt().WithName("foo").WithVersion("v01.02.3-a").V(),
			wantErr:     true,
		},
		{
			name:        "no platform specified",
			receiptName: "foo",
			receipt:     testutil.NewReceipt().WithName("foo").WithPlatforms().V(),
			wantErr:     true,
		},
		{
			name:        "empty file operations",
			receiptName: "foo",
			receipt: testutil.NewReceipt().WithName("foo").WithPlatforms(
				testutil.NewPlatform().WithFiles([]index.FileOperation{}).V()).V(),
			wantErr: true,
		},
		{
		...
		},
		{
			name:        "short description with line break",
			receiptName: "foo",
			receipt:     testutil.NewReceipt().WithShortDescription("just foo\n").V(),
			wantErr:     true,
		},
		{
			name:        "short description with carriage return",
			receiptName: "foo",
			receipt:     testutil.NewReceipt().WithShortDescription("just foo\r").V(),
			wantErr:     true,
		},
		{
			name:        "short description with CRLF",
			receiptName: "foo",
			receipt:     testutil.NewReceipt().WithShortDescription("just\r\nfoo").V(),
			wantErr:     true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := ValidateReceipt(tt.receiptName, tt.receipt); (err != nil) != tt.wantErr {
				t.Errorf("ValidateReceipt(%s) error = %v, wantErr %v", tt.receiptName, err, tt.wantErr)
			}
		})
	}
}
